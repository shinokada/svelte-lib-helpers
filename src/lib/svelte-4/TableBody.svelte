<script lang="ts" generics="T">
  ...
  let filter = getContext('filter') as Writable<
    ((t: T, term: string) => boolean) | null
  >;
  let searchTerm = getContext('searchTerm') as Writable<string>;
  $: filtered = $filter
    ? items.filter((item) => $filter(item, $searchTerm))
    : items;
  let sorter = getContext('sorter') as Writable<{
    id: string;
    sort: (a: T, b: T) => number;
    sortDirection: -1 | 1;
  } | null>;
  $: sorted = $sorter
    ? filtered.toSorted((a, b) => $sorter.sortDirection * $sorter.sort(a, b))
    : filtered;
</script>
