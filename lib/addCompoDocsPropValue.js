...
export const addCompoDocsPropValue = (srcDir) => {
  ...
  const repoName = packageJson.name || 'flowbite-svelte-next';
  
  // Build GitHub URL base
  const githubBaseUrl = `https://github.com/themesberg/${repoName}`;
  ...
  const processFile = (filePath) => {
    ...
    if (propMatch) {
      ...
      if (typeName) {
        typeSection = `\n## Type\n${typeName}`;
        if (typeLineNumber) {
          // Use the GitHub URL format with themesberg organization
