...
function extractExtendedTypes(content, targetInterface) {
  ...
  // Function to extract props from an interface or type
  ...
  // Try to match the target interface or type
  const regex = new RegExp(`(?:export\\s+)?(?:interface|type)\\s+(${targetInterface})\\s*(?:extends\\s+([\\w\\s,<>]+))?\\s*(?:=\\s*([\\w\\s&]+))?\\s*{([\\s\\S]*?)}`, 'm');
  ...
  const [, interfaceName, extendedInterfaces, intersectionTypes, ownProps] = match;
  console.log(`Found interface/type: ${interfaceName}`);

  let allProps = extractProps(ownProps);
  ...
  // Remove duplicates and handle Omit
  const finalProps = [];
  const seenProps = new Set();
  for (const prop of allProps) {
    if (!seenProps.has(prop.name) && !prop.name.startsWith('Omit<')) {
...
const componentData5FromType = (srcDir) => {
  ...
  function processFile (filePath) {
    ...
    if (fs.existsSync(tsFilePath)) {
      ...
      tsTypes = extractExtendedTypes(tsContent, targetInterface) || { props: [] };
