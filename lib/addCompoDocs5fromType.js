...
// Helper function to extract types from TypeScript file
function extractTypesFromTS(filePath) {
  console.log(`Extracting types from: ${filePath}`);
  try {
    const content = fs.readFileSync(filePath, 'utf-8');
    console.log(`File content:\n${content}`);
    const interfaceMatch = content.match(/interface\s+CheckboxProps[^{]*{([\s\S]*?)}/);
    if (interfaceMatch) {
    ...
    console.error(`Error reading TS file: ${error.message}`);
  }
  return [];
}
...
export const addCompoDocs5fromType = (srcDir) => {
  ...
  const processFile = (filePath) => {
    ...
    // Extract import statement for Props
    ...
    if (importMatch) {
      ...
      if (fs.existsSync(tsFilePath)) {
        tsTypes = extractTypesFromTS(tsFilePath);
