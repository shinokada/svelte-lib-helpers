...
/**
 * Splits a props interface string into an array of objects representing individual props.
 * @param {string} propsInterface - The string representation of the props interface.
 * @returns {array} - An array of objects with properties `name` and `type`.
 */
...
export const addCompoDocs5fromType = (srcDir) => {
  ...
  /**
   * Processes a single Svelte component file, extracting props and generating documentation comments.
   * @param {string} filePath - The path to the Svelte component file.
   */
  const processFile = (filePath) => {
    ...
    const propLines = content.match(/let {([^}]*)}: Props/);
    ...
    combinedProps.forEach((cp, idx) => {
      ...
      if (defaultValue) {
        ...
        // console.log('combinedProps[idx]', combinedProps[idx]);
      }
    });
