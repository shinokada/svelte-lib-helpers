...
export async function componentDataPropValue(githubLink) {
  // Validate required argument
  if (!githubLink) {
    throw new Error('GitHub link is required. Usage: componentDataPropValue("organization/repo")');
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  try {
    componentDataPropValue(githubLink);
  } catch (error) {
