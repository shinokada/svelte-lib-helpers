...
export async function componentDataPropValue(srcDir, githubLink) {
  ...
  const processFile = async (filePath) => {
    ...
    // Special debugging for Checkbox component
    if (name === 'Checkbox') {
      console.log('Checkbox component found!');
      console.log('Type name:', typeName);
      console.log('Props count:', props.length);
      console.log('Is external type:', isExternalType);
      console.log('Source module:', sourceModule);
      
      // Log the file content for further analysis
      const content = fs.readFileSync(filePath, 'utf8');
      console.log('First 500 chars of Checkbox.svelte:');
      console.log(content.substring(0, 500));
    }
    
    // Handle the case when labelProps in Checkbox doesn't get properly parsed
    if (name === 'Checkbox') {
      const labelPropsIndex = props.findIndex(p => p[0] === 'labelProps');
      if (labelPropsIndex !== -1 && props[labelPropsIndex][1] === '{') {
        // Fix the default value
        props[labelPropsIndex][1] = '{}';
      }
    }
